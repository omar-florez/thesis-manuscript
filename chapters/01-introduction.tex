...
of letters each representing an amino acid. There are twenty (20) known amino
acids considered to be the building blocks of protein. When arranged into a
sequence, the order of appearance becomes important. Although prediction can
be done from protein sequences (\cite{whisstock2003prediction,
devos2000practical}), this work will focus on using expressions as description
for proteins. 
...
phylogenetic profiles and DNA chips. Proteins are represented as a vector of
...
\par Proteins can belong to one or more functional categories. These
categories can refer to various life processes (e.g. metabolism, cell
reproduction, synthesis, etc) and are arranged in an ontology. The word 
\textit{ontology} may have philosophical roots, but organizing categories is
convenient to observe the relationships and  properties of protein functions
\textemdash and it turns out to be one of the  standard tools in
bioinformatics.
...
(\mathbf{h})$. Take note that the size of $\mathbf{h}$ may not necessarily be
the same as $\mathbf{X}$. In sum, what the autoencoder does is that it takes an
input matrix and faithfully  reconstructs it given a bottleneck (the size of 
$\mathbf{h}$). This may sound trivial, but due to the \textit{encoding layer} 
$\mathbf{h}$, interesting structure is learned from the data. The learned 
representations $\mathbf{h}$ are treated as the new input $\mathbf{\widehat{X}}$ 
(similar to the \texttt{XOR} problem) for a classifier. With a good set-up and 
optimization scheme, it is even possible to compress data using this technique 
(\cite{theis2017lossy}).
...
classification, one shortcoming of this technique is that it tends to learn
trivial representations from sparse and high-dimensional data
(\cite{wang2017feature, chen2017kate}) characteristic of proteins. This is
usually the case when the optimization task is to learn an approximation to
the identity $\mathbf{X} \approx \mathbf{\widehat{X}}$, which may favor
irrelevant features due to overfitting instead of learning a generalized
pattern or representation. It is then important to motivate the autoencoder
in creating relevant features to accomplish the classification task.
...
the input data. This method is task-specific for protein function prediction
because only a subset of features from protein data can be considered useful
(\cite{iqbal2014efficient, gaudet2017gene}). The proposed method will be trained
on two protein benchmark datasets, and the extracted features will be used as
input to a binary relevance classifier. It will be compared against a baseline
without feature extraction, and opposed to existing feature extraction techniques
in literature.
...
\noindent Thorough explanation of the proposed method will be done in Chapter
\ref{Methodology} while the results supporting the answers to the questions
above will be discussed in Chapter \ref{Results}.
